...
<style lang="less">
  ...
  @primary-color-bind: v-bind('token.colorPrimary');
  ...
  .@{menu-prefix-cls} {
    ...
    &-dark&-vertical &-submenu-title {
      ...
      &-active:not(.@{menu-prefix-cls}-submenu) {
        background-color: @primary-color-bind !important;
      }
    }

    // 白色菜单栏会使用这个css
    // todo 原版会有10%的透明度 需要用js来做
    &-light&-vertical &-item {
      &-active:not(.@{menu-prefix-cls}-submenu) {
        background-color: v-bind('fadePrimaryColor') !important;
        color: @primary-color-bind;
      }
    }
  }
