...
<script lang="ts" setup>
  ...
  async function customRequest(info: UploadRequestOption<any>) {
    ...
    try {
      ...
      //   // //console res.ossId 数据类型
      //   // console.log(typeof res.ossId);
      //   // //console res
      //   // console.log(res)
    ...
    const list = (fileList.value || [])
      ...
      .map((item: any) => {
        if (item?.response && props?.resultField) {
          console.log("match resultField")
          console.log(item?.response?.[props.resultField])
        ...
        console.log(item?.response?.url + " or " + item?.url)
        return  item?.url || item?.response?.url;
      });
    return list;
  }
