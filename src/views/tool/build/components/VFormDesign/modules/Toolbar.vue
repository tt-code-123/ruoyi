...
<template>
  <div class="operating-area">
    <!-- 头部操作按钮区域 start -->
    <!-- 操作左侧区域 start -->
    <div class="left-btn-box ml-24px flex gap-8px items-center h-full">
      <Tooltip v-for="item in toolbarsConfigs" :title="item.title" :key="item.icon">
        <a-button @click="$emit(item.event)" class="toolbar-text">
          <Icon :icon="item.icon" />
        </a-button>
      </Tooltip>
      <Divider type="vertical" />
      <Tooltip title="撤销">
        <a-button :class="{ disabled: !canUndo }" :disabled="!canUndo" @click="undo">
          <Icon icon="ant-design:undo-outlined" />
        </a-button>
      </Tooltip>
      <Tooltip title="重做">
        <a-button :class="{ disabled: !canRedo }" :disabled="!canRedo" @click="redo">
